...
def rt_fine_tuning(sentences_train, sentences_test, targets_train, targets_test):
    """
        fine tune the model for real time training 
    ...
    # Model reconstruction from JSON file
    with open(c.FT_MODEL_JSON_FILE_NAME, 'r') as f:
        json = f.read()
    model = model_from_json(json, custom_objects={'KerasLayer': hub.KerasLayer})
...
def predict_top_k_hashtags(sentences, k, trending_map, timestamp):
    """
    ...
    result = [word_embedding_model.retain_hts(emb_model.wv.similar_by_vector(h_vect, topn=top_n_words))[:k] for h_vect
              in h_list]
    result = sorted(map(lambda tup: (tup[0], tup[1] * exp(-LAMBDA * (timestamp - trending_map[tup[0]]))), result),
